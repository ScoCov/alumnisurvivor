class_name Wander_Ignore_State
extends Enemy_Wander_Movement_State

var target: Vector2

func update(_delta: float):
	if enemy_movement_component.movement_type is not EnemyMovementWanderIgnore:
		return
		#print("Movement Type: %s" % enemy_movement_component.movement_type.name)
